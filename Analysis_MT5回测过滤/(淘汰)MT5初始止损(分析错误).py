...
file = __mypath__.get_desktop_path() + "\\海龟交易法则研究.EURUSD.H1.xlsx"
...
#%% #############################
tf_stoploss = "TIMEFRAME_M5"
new_unit_total = myMT5Report.maxSL_matching_unit(unit=unit_total, symbol=symbol, timefrom=timefrom, timeto=timeto, tf_report=timeframe, tf_stoploss=tf_stoploss)
...
# ---得到报告过滤的策略结果
result
result_filter, x_min, x_max = myMT5Report.get_report_filter_result(new_unit=new_unit_total, mode="1side")

# ---画报告过滤的结果。参数中 new_unit 为 indi_matching_unit() 的输出结果。data为报告对应的原数据；mode="range"范围过滤；"2side"两侧过滤；"1side"单侧过滤；
myMT5Report.plot_report_filter_analysis(data=data, new_unit=new_unit_total, mode="1side",savefig=None, batch=False)
