...
mylogging = MyDefault.MyClass_Default_Logging(activate=True, filename=__mypath__.get_desktop_path()+"\\海龟反转策略参数优化.log") # 日志记录类，需要放在上面才行
...
# 策略说明：
'''
# 反转思维来考虑公开版海龟交易策略，运用唐奇安通道突破来反向入场，其中唐奇安通道本质就是bar之前的N个周期极值：
# 只考虑入场，出场模式放在其他地方考虑。
# 向上突破通道，close大于通道上轨，触发做空信号；向下突破通道，close小于通道下轨，触发做多信号。
...
opt.total_folder = "F:\\工作---策略研究\\公开的海龟策略\\_海龟反转研究"
