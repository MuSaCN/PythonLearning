...
# 2.筛选最佳的占优策略。思路：模式1：先分析词缀sharpe和cumRet下是否有指定比率(比如70%)领先者，若有则领先者为最佳，否则进入模式2；模式2：先分析某个词缀(比如sharpe)下哪个策略的优势超过指定比率(比如60%)，该策略得1分。对所有词缀进行分析，若某个策略的得分最大且超过指定数量(词缀个数*2*60%)，则该策略认为是最佳的占优策略。
'''
...
#%% ******修改这里******
s_better.strategy_para_name = ["k", "holding", "lag_trade"]
s_better.symbol_list = ["USDJPY"]# myMT5Pro.get_main_symbol_name_list()
s_better.total_folder = "F:\\工作---策略研究\\1.简单的动量反转\\_动量研究"
